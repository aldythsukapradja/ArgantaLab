...
export function MediaCenter() {
  ...
  async function onGenerate(opts: { force?: boolean; prompt?: string; kind?: Kind; stage?: number; silent?: boolean } = {}) {
    ...
    const runId = crypto.randomUUID() // shared with saveMediaAsset below — the run_id↔asset lineage link
    const logMediaRun = (r: any) => {
      const prov = r.provenance || {}
      return logAgentRun({
        runId, domain: 'media', task: k,
    ...
    // copy real generated bytes into the media-artifacts bucket, linked to this
    // run_id. MUST run after the agent_runs row lands (its run_id is a foreign
    // key) — callers await logMediaRun() first. Best-effort beyond that — a
    // save failure never blocks the UI; the asset just stays local-only.
    const saveImageArtifact = (r: any) => {
      const prov = r.provenance || {}
      const bytes: Uint8Array | undefined = r.output?.bytes
      if (!bytes) return
      saveMediaAsset({
        runId, kind: 'image', bytes, mime: r.output?.mime || 'image/png',
        prompt: text, provider: prov.provider || 'unknown', model: prov.model ?? null, costUsd: prov.cost ?? 0,
      })
    }
    try {
      let res: any
      if (REAL.has(k)) {
        ...
        if (k === 'image' && st >= 1) {
          ...
          if (g) {
            ...
            setResult(res)
            await logMediaRun(res)
            saveImageArtifact(res)
      ...
      await logMediaRun(res)
      if (k === 'image' && res.status === 'succeeded') saveImageArtifact(res)
