// R3 — Video Studio: one surface, two modes. Generate (Runway-shaped sovereign
// Wan renders) and Edit (the existing, untouched timeline builder). "Send to
// Edit" persists a generated clip to the media library so the timeline's own
// media drawer picks it up — reusing the working editor without modifying it.
...
export function VideoStudio() {
  ...
  async function sendToEdit(clip: { url: string; meta: any }) {
    setMode('edit')
    if (!cloudEnabled) { setNote('Clip kept in the Generate gallery — sign in to add it to the timeline library.'); return }
    try {
