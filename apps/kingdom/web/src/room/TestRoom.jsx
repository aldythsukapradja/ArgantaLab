...
import { authAvailable } from '../net/account.js';
...
export default function TestRoom({ spec, account }) {
    ...
    (async () => {
      ...
      const linked = links
        ...
        .map((l) => {
          ...
          return mon ? { name: mon.name, xp: mon.defaultExperience || 0, mobId: l.mobId, paletteId: l.paletteId } : null;
        })
        .filter(Boolean)
  ...
  useEffect(() => {
    ...
    async function addRemote(id, meta) {
      ...
      const remote = {
        ...
        hp: meta.hp ?? 100, maxHp: 100, resources: null,
      };
      g.remotes[id] = remote;
    ...
    const net = joinArena({
      me: {
        ...
        spec, tile: g.player.tile, facing: g.player.facing, hp: g.player.hp,
      },
      onPeers: (peers) => {
        ...
        if (ev.type === 'move' && r) {
          ...
          if (dist <= 1) {
            ...
            if (p.hp <= 0) {
              ...
              p.tile = [8, 13]; p.from = [...p.tile]; p.moveT = 1; p.hp = 100;
    ...
    g.net = net;
    return () => { net.leave(); g.net = null; g.remotes = {}; };
  }, [ready, account?.character?.id]);
  ...
  function strike(g) {
    ...
    setTimeout(() => {
      const m = g.monsters.find((m_) => m_.state !== 'die' && m_.tile[0] === tx && m_.tile[1] === ty);
      if (!m) return;
      m.hp -= 34;
      if (m.hp <= 0) {
  ...
  function drawNameplate(ctx, ent, [px, py], isMe) {
    ...
    const badge = (isMe ? ent.accountType : ent.meta?.accountType) === 'kid' ? '🧒' : '👑';
        ...
        <div className="browser-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) setShowSettings(false); }}>
          <div className="settings">
            ...
            <div className="settings-body">
              ...
              <section>
                <h4>Skill slots (effect id 0–647)</h4>
                {skills.map((s, i) => (
                  <div className="setrow" key={i}>
                    <label>Skill {i + 1}</label>
                    <input type="number" min="0" max="647" value={s.fx}
                      onChange={(e) => setSkills((arr) => arr.map((x, j) => j === i ? { fx: Number(e.target.value) } : x))} />
