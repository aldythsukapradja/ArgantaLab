...
import { nodeState, nodeUnlocked, nodeDoneToday } from '@lib/learnProgress'
...
interface Meta { fi: number; st: { status: 'open' | 'done'; stars: number }; doneToday: boolean; unlocked: boolean; isBoss: boolean; isChest: boolean; isLesson: boolean; isCurrent: boolean }
...
function useMeta(world: World) {
  ...
  const info = (node: JourneyNode, currentKey?: string): Meta => {
    ...
    // `doneToday` (not ever-done) drives the glyph/stars, so the trail looks
    // fresh each day; `unlocked` still rides on permanent status.
    return { fi, st, doneToday: nodeDoneToday(world.key, node.key), unlocked: nodeUnlocked(world, fi), isBoss: node.type === 'boss', isChest: node.type === 'chest', isLesson: node.type === 'lesson', isCurrent: node.key === currentKey }
  }
  return { flat, info }
}
...
const SKINS: Record<Skin, { head: string; sub: string; icon: (m: Meta) => string; deco: JSX.Element }> = {
  arcade: {
    ...
    icon: m => m.doneToday ? '★' : !m.unlocked ? '🔒' : m.isBoss ? '🔥' : m.isChest ? '🎁' : '▶',
    ...
    <div className={`jr jr-${skin}`} style={{ ['--wc' as string]: p.world.color }}>
      ...
      <div className="jr-track">
        {p.world.units.map(unit => {
          const done = unit.nodes.filter(n => nodeDoneToday(p.world.key, n.key)).length
          unitIdx++
          return (
              ...
              {unit.nodes.map(node => {
                ...
                return (
                  <div key={node.key} className={`jr-row ${side}${m.isCurrent ? ' current' : ''}${m.isBoss ? ' boss' : ''}`} style={{ ['--d' as string]: `${(m.fi % 8) * 70}ms` }}>
                    ...
                    <button className={`jr-node${m.doneToday ? ' done' : ''}${!m.unlocked ? ' locked' : ''}${m.isBoss ? ' boss' : ''}${m.isChest ? ' chest' : ''}${m.isLesson ? ' lesson' : ''}${m.isCurrent ? ' current' : ''}`}
                      ...
                      {m.doneToday && m.st.stars > 0 && <span className="jr-stars">{'★'.repeat(m.st.stars)}</span>}
                    </button>
                    {m.isCurrent && (
