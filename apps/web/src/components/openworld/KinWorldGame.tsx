  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const gatePos = GATES.map((g, i) => { const a = (-90 + i * 60) * Math.PI / 180; return { ...g, x: CX + Math.cos(a) * 380, y: CY + Math.sin(a) * 300 } })
        const clamp = (v: number, a: number, b: number) => Math.max(a, Math.min(b, v))
        const world = new PIXI.Container(); app.stage.addChild(world)

        // grass — drawn directly (no generateTexture/TilingSprite, which were fragile)
        const grass = new PIXI.Graphics()
        grass.rect(0, 0, W, H).fill(0x7ec850)
        ...
        gatePos.forEach(g => {
          ...
          c.hitArea = new PIXI.Rectangle(-29, -32, 58, 60); c.on('pointertap', () => cbRef.current.onEnterDungeon(g.world))
          const gr = new PIXI.Graphics()
          gr.roundRect(-29, -32, 58, 60, 16).fill(g.hex).stroke({ width: 3, color: 0xffffff })
          gr.roundRect(-18, -16, 36, 44, 12).fill(0x2a2740)
          gr.rect(25, -48, 3, 22).fill(0x8a5a2b); gr.rect(28, -48, 17, 11).fill(g.hex)
          c.addChild(gr)
          const em = new PIXI.Text({ text: g.emoji, style: { fontSize: 22 } }); em.anchor.set(0.5); em.position.set(0, -2); c.addChild(em)
          world.addChild(c)
          addLabel(PIXI, world, g.name, g.x, g.y + 40, 16)
        })

        // mutable scene state declared up-front so the ticker can run immediately
        // (the static scene renders now; kin/hero sprites pop in once rasterised)
        const bobbers: { s: any; baseY: number; t: number }[] = []
        let hero: any = null
        let nearWorld: string | null = null
        window.addEventListener('keydown', kd); window.addEventListener('keyup', ku)

        app.ticker.add((tk: any) => {
          if (destroyed) return
          const dt = tk.deltaTime
          // fit whole world into the viewport (no scroll / panning)
          const z = Math.min(app.screen.width / W, app.screen.height / H) || 1
          world.scale.set(z); world.position.set((app.screen.width - W * z) / 2, (app.screen.height - H * z) / 2)
          // bob the guard kin
          bobbers.forEach(b => { b.t += 0.04 * dt; b.s.y = b.baseY - Math.abs(Math.sin(b.t)) * 5 })
        ...
        for (let i = 0; i < guards.length; i++) {
          ...
          const s = new PIXI.Sprite(PIXI.Texture.from(canvas)); s.anchor.set(0.5); s.position.set(gx, gy); s.scale.set(0.6)
          world.addChild(s); bobbers.push({ s, baseY: gy, t: Math.random() * 6 })
        }
