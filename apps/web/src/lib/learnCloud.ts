...
// Cross-device sync for the learn engine. The engine is local-first (reads/writes
// localStorage for instant offline play); this mirrors the two state blobs to a
// single Supabase row per user. All calls degrade silently if the backend isn't
// reachable, so the app always works on localStorage alone.

const NODES_KEY = 'argantalab_nodes_v1'
const MASTERY_KEY = 'argantalab_mastery_v1'

type Dict = Record<string, Record<string, unknown>>
function read(k: string): Dict { try { return JSON.parse(localStorage.getItem(k) || '{}') } catch { return {} } }
function write(k: string, v: Dict) { try { localStorage.setItem(k, JSON.stringify(v)) } catch { /* ignore */ } }
