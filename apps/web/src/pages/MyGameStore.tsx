...
export default function MyGameStore() {
  ...
  const publish = (g: SavedGame) => {
    const updated = { ...g, published: !g.published }
    setMine(saveMyGame(updated))
    if (uid) { pushGame(uid, updated); setGameVisibility(uid, g.id, updated.published ? 'public' : 'private', learnerName) }
    addToast(updated.published ? `“${g.title}” is now public!` : `“${g.title}” is private`, updated.published ? '🌍' : '🔒')
  }
  const copy = async (g: SavedGame) => {
    try { await navigator.clipboard.writeText(`${origin}/play/${g.id}`) } catch { /* ignore */ }
                ...
                <div className="store-body">
                  ...
                  {g.published && (
                    <div className="store-link">
                      <code>{origin.replace(/^https?:\/\//, '')}/play/{g.id}</code>
