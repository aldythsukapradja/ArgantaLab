...
import { generateVideo, generateI2V, processV2V, uploadFile } from "../fabric.js";
import {
  t2vModels,
  i2vModels,
  v2vModels,
...
export default function VideoStudio({
  ...
  const processDroppedImage = async (file) => {
    ...
    setImageProgress(0);
    try {
      // Sovereign (local) mode: read the image as a data URL client-side and
      // animate it on the local GPU — no muapi upload (which needs a paid key).
      if (apiKey === "local") {
        ...
        setUploadedImageUrls([dataUrl]);
        setUploadedVideoUrl(null);
        setUploadedVideoName(null);
        setV2vMode(false);
        if (!imageMode) {
          const sov = i2vModels.find((m) => m.id === "arganta-sovereign-i2v") || i2vModels[0];
          setImageMode(true);
          setSelectedModel(sov.id);
          setSelectedModelName(sov.name);
          applyControlsForModel(sov.id, true, false);
        }
