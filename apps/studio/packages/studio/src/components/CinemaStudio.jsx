...
import { generateImage, uploadFile, SOVEREIGN_T2I_ID } from "../fabric.js";
import { MOVES, compileCameraClause } from "../cameraGrammar.js";
...
export default function CinemaStudio({
  ...
  // ── Dropdown state ──
  const [openDropdown, setOpenDropdown] = useState(null); // 'ar' | 'res' | 'move' | null
  ...
  const handleGenerate = useCallback(async () => {
    ...
    const withMove = [basePrompt, compileCameraClause({ move: settings.move })]
      .filter(Boolean)
      .join(", ");
    ...
    try {
      // Text→image with no paid key → render free on the local GPU (sovereign);
      // the camera grammar is just prompt text, so ComfyUI honors it. Reference-
      // image edits still need the paid model (sovereign i2i isn't wired yet).
      const localOnly = apiKey === "local";
      const model = uploadedImage
        ? "nano-banana-pro-edit"
        : (localOnly ? SOVEREIGN_T2I_ID : "nano-banana-pro");
  ...
  const loadHistoryItem = (entry, idx) => {
    if (entry.settings) {
      setSettings((prev) => ({
        ...
        move: entry.settings.move ?? prev.move,
        ...
        <div className="w-full bg-gradient-to-b from-[#18181c]/90 via-[#0f0f12]/90 to-[#0c0c0e]/95 backdrop-blur-2xl rounded-[2rem] border border-white/[0.08] p-4 flex flex-col gap-3 shadow-[0_15px_50px_rgba(0,0,0,0.8)]">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 pt-3 border-t border-white/[0.03] relative">
            <div className="flex items-center gap-2 relative flex-wrap pb-1 md:pb-0">
              ...
              {/* Camera Move Button — motion grammar, the "what would this shot DO" control */}
              <div className="relative">
                <button
                  ref={moveBtnRef}
                  className="h-[34px] flex items-center gap-2 px-3.5 bg-[#16161a]/60 hover:bg-[#202026]/80 rounded-md transition-all border border-white/[0.06] group whitespace-nowrap shadow-inner text-[11px] font-semibold text-white/70 hover:text-white"
                  onClick={() => setOpenDropdown((d) => (d === "move" ? null : "move"))}
                  title="Camera move — motion grammar"
                >
                  <span className="opacity-60">{MOVES.find((m) => m.id === settings.move)?.glyph ?? "▢"}</span>
                  {MOVES.find((m) => m.id === settings.move)?.label ?? "Static"}
                </button>
                {openDropdown === "move" && (
                  <div className="absolute bottom-[calc(100%+12px)] left-0 z-50 bg-[#0c0c0f]/95 rounded-xl p-2 shadow-[0_10px_40px_rgba(0,0,0,0.8)] border border-white/[0.08] backdrop-blur-2xl min-w-[220px] max-h-[320px] overflow-y-auto custom-scrollbar">
                    {MOVES.map((m) => (
                      <button
                        key={m.id}
                        onClick={() => {
                          setSettings((prev) => ({ ...prev, move: m.id }));
                          setOpenDropdown(null);
                        }}
                        className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-left text-[12px] transition-colors ${
                          settings.move === m.id ? "bg-[#22d3ee]/15 text-[#22d3ee]" : "text-white/70 hover:bg-white/5 hover:text-white"
                        }`}
                      >
                        <span className="w-5 text-center opacity-70">{m.glyph}</span>
                        {m.label}
                      </button>
                    ))}
                  </div>
