...
// ──────────────────────────────────────────────────────────────
//  GENERAL DECK — the cinematic camera flight, exactly as before. Wrapped only
//  for the launcher: shared theme + a way back to the deck gallery.
// ──────────────────────────────────────────────────────────────
  ...
  return (
    <div className="cosmos-app">
      ...
      <nav className="cosmos-nav">
        <button className="cnav-brand" onClick={onExit} aria-label="Back to decks"><ArgantaMark /><span>Arganta</span></button>
        <div className="cnav-right">
          {view.mode !== 'hub' && <button className="cnav-map" onClick={toMap}><IconMap /> map</button>}
          <a className="cnav-inv" href="#" onClick={e => { e.preventDefault(); goFlight('company') }}>for investors →</a>
          <ThemeToggle className="cnav-theme" />
        </div>
      </nav>
