...
import {
  ...
  History, Pencil, Presentation, RotateCcw, Search, ShieldCheck, Sparkles, Target, Trophy,
  Upload, Users, WandSparkles, X, Zap,
} from 'lucide-react';
...
import {
  ...
  downloadText, liveSlides, slugify,
} from './materials';
import { currentRevision, useContent } from './content-store';
import { applyImport, downloadBlob, exportDeckPptx, proposeImport, type ImportProposal } from './deck-io';
import { docxBlob } from './officegen/docx-writer';
...
function Presenter({ day, onClose }: { day: CourseDay; onClose: () => void }) {
  ...
  // Subscribing to the content store means an edit made on the web — or a deck
  // re-imported from PowerPoint — is what actually gets presented.
  useContent();
  const slides = liveSlides(day);
  const current = slides[Math.min(slide, slides.length - 1)];
  const go = (delta: number) => setSlide((n) => (n + delta + slides.length) % slides.length);
  ...
  return (
    <div className="fc-presenter" role="dialog" aria-modal="true" aria-label={`${day.title} presentation`}>
      ...
      <div className="fc-presenter-body">
        <aside>
          {slides.map((s, i) => <button key={s.id} className={i === slide ? 'active' : ''} onClick={() => setSlide(i)}><small>{String(i + 1).padStart(2, '0')}</small><span>{s.eyebrow ?? (s.kind === 'opaque' ? 'POWERPOINT' : '')}</span><b>{s.title ?? s.opaqueLabel ?? 'Untitled slide'}</b></button>)}
        </aside>
        <main>
          <div className={`fc-slide ${current.layout ?? 'concept'}`} style={{ '--day': day.color } as CSSProperties}>
            ...
            <footer><span>THE VOLVE MISSION · FROM DISCOVERY TO DECISION</span><b>{slide + 1} / {slides.length}</b></footer>
          </div>
          {notes && <div className="fc-speaker-note"><BookMarked size={14} /><div><b>Facilitator note</b><p>{current.note}</p></div></div>}
          <div className="fc-presenter-nav"><button onClick={() => go(-1)}><ChevronLeft size={18} /></button><span>{slide + 1} of {slides.length}</span><button onClick={() => go(1)}><ChevronRight size={18} /></button></div>
        </main>
      </div>
    </div>
  );
...
function MaterialPreview({ material, day, onClose, onPresent }: { material: Material; day: CourseDay; onClose: () => void; onPresent: () => void }) {
  ...
  // buildMaterialDoc reads slides via getDeck(), which is not React state — so
  // the memo must depend on the content store's own snapshot, not just on
  // material/day, or a web edit or PowerPoint import never appears here.
  const doc = useMemo(() => buildMaterialDoc(material, day), [material, day, content]);
  ...
  const isDeck = material.kind === 'Presentation';
  const revision = isDeck ? currentRevision(material.id) : undefined;
  ...
  return (
    <div className="fc-modal-backdrop" onMouseDown={(e) => e.currentTarget === e.target && onClose()}>
      <div className="fc-material-modal" role="dialog" aria-modal="true">
        <header>
          <span style={{ '--day': day.color } as CSSProperties}><Icon size={18} /></span>
          <div><small>{material.kind.toUpperCase()} · DAY {day.number} · v{doc.version}{revision ? ` · rev ${revision.n}` : ''}</small><b>{material.title}</b></div>
          {isDeck && (
            <div className="fc-material-header-actions">
              <button onClick={() => setEditing(true)}><Pencil size={13} /><span>Edit</span></button>
              <button onClick={() => setHistory(true)}><History size={13} /><span>History</span></button>
            </div>
          )}
        ...
        <div className="fc-material-preview">
          ...
          <div className="fc-material-info">
            ...
            <div className="fc-modal-actions">
              {isDeck && <button className="primary" onClick={onPresent}><Play size={14} />Start presentation</button>}
              {isDeck ? (
                <>
                  <button onClick={() => { void exportDeckPptx(material.id).catch((e) => setIoError(String(e.message ?? e))); }}>
                    <Download size={14} />Export .pptx
                  </button>
                  <button onClick={() => fileRef.current?.click()}><Upload size={14} />Import .pptx</button>
                  <input
                    ref={fileRef} type="file" accept=".pptx" hidden
                    onChange={(e) => {
                      const f = e.target.files?.[0];
                      e.target.value = '';
                      if (f) { setBusy(true); setIoError(null); void proposeImport(f, material.id).then(setProposal).catch((err) => setIoError(String(err.message ?? err))).finally(() => setBusy(false)); }
                    }}
                  />
                </>
              ) : (
                <button onClick={() => { void docxBlob(doc, 'The Volve Mission · From Discovery to Decision').then((b) => downloadBlob(`${slugify(material.title)}.docx`, b)); }}>
                  <Download size={14} />Export .docx
                </button>
              )}
              <button onClick={() => downloadText(`${slugify(material.title)}.md`, docToMarkdown(doc))}><FileText size={14} />Markdown</button>
            </div>
            {busy && <p className="fc-io-note">Reading the PowerPoint file…</p>}
            {ioError && <p className="fc-io-error">{ioError}</p>}
          </div>
        </div>
      </div>
      {proposal && <ImportReview proposal={proposal} onCancel={() => setProposal(null)} onApply={() => { void applyImport(proposal).then(() => setProposal(null)); }} />}
      {editing && isDeck && <DeckEditor materialId={material.id} dayNumber={day.number} onClose={() => setEditing(false)} />}
      {history && isDeck && <VersionHistory materialId={material.id} onClose={() => setHistory(false)} />}
    </div>
  );
}

/** Nothing enters the history until the trainer has seen exactly what changes. */
function ImportReview({ proposal, onCancel, onApply }: { proposal: ImportProposal; onCancel: () => void; onApply: () => void }) {
  const s = proposal.summary;
  const nothing = !s.details.length && !s.reordered;
  return (
    <div className="fc-modal-backdrop" onMouseDown={(e) => e.currentTarget === e.target && onCancel()}>
      <div className="fc-import-modal" role="dialog" aria-modal="true" aria-label="Review PowerPoint import">
        <header>
          <span><Upload size={17} /></span>
          <div><small>REVIEW IMPORT</small><b>{proposal.doc.slides.length} slides from PowerPoint</b></div>
          <button onClick={onCancel} aria-label="Cancel import"><X size={16} /></button>
        </header>

        {proposal.staleBase && (
          <div className="fc-import-warn stale">
            <ShieldCheck size={14} />
            <div>
              <b>This deck was exported from an older revision.</b>
              <span>Built from {proposal.staleBase.base}, but {proposal.staleBase.current} is live. Importing replaces the current deck, so anything changed on the web since then will be superseded.</span>
            </div>
          </div>
        )}
        {proposal.warnings.map((w) => (
          <div className="fc-import-warn" key={w}><MoreHorizontal size={14} /><span>{w}</span></div>
        ))}

        <div className="fc-import-diff">
          <div className="fc-import-tally">
            <span className="add">+{s.added} added</span>
            <span className="edit">{s.edited} edited</span>
            <span className="del">-{s.removed} removed</span>
            {s.reordered && <span className="move">reordered</span>}
          </div>
          {nothing
            ? <p className="fc-import-none">This file matches the live deck exactly. Nothing would change.</p>
            : (
              <ul>
                {s.details.map((d) => (
                  <li key={d.slideId + d.kind} className={d.kind}>
                    ...
                    <div>
                      <b>{d.title}</b>
                      {d.fields?.length ? <small>changed: {d.fields.join(', ')}</small> : <small>{d.slideId}</small>}
                    </div>
                  </li>
                ))}
              </ul>
        ...
        <footer>
          ...
          <button className="primary" onClick={onApply} disabled={nothing}>
            <Check size={14} />Apply as new revision
          </button>
        </footer>
      </div>
    </div>
...
export function Fieldcraft({ onOpenWorkspace }: { onOpenWorkspace: (id: WorkspaceId, module?: string) => void }) {
  ...
  return (
    <div className="fc-page fc-home">
      <section className="fc-hero">
        ...
        <aside className="fc-ready-panel"><div className="fc-ready-top"><span><ShieldCheck size={15} />DELIVERY STATUS</span><b>Course ready</b></div><div className="fc-ready-score"><ProgressRing value={92} /><div><b>92%</b><span className="fc-ready-caption">material readiness</span></div></div><div className="fc-ready-list"><span><Check size={13} />5 day agendas</span><span><Check size={13} />30 presentation slides</span><span><Check size={13} />8 guided labs</span><span><Check size={13} />20 trainer resources</span><span className="review"><MoreHorizontal size={13} />Final exam in review</span></div><button onClick={onTrainer}><CirclePlay size={15} />Launch Day 1</button></aside>
      </section>
...
function DayLearn({ day, onPresent, onOpenWorkspace }: { day: CourseDay; onPresent:()=>void; onOpenWorkspace:(id:WorkspaceId,module?:string)=>void }) {
...
function Labs({ days, onOpenWorkspace }: { days: CourseDay[]; onOpenWorkspace:(id:WorkspaceId,module?:string)=>void }) {
  const session = useSession();
  const completed = MISSIONS.filter((m)=>isMissionComplete(session,m.id)).length;
  // Launch straight onto the module the first step happens in, so the learner
  // lands in the tool rather than at the front door of the vertical.
  const launch = (missionId: string, workspace: WorkspaceId, firstModule?: string) => {
    startMission(missionId);
    onOpenWorkspace(workspace, firstModule);
  };
  return <div className="fc-subpage fc-labs-page"><section className="fc-page-title"><div><span>GUIDED APP MISSIONS</span><h1>Practice inside the real lifecycle workspaces.</h1><p>Every lab pins a data scope, walks ordered steps and captures evidence as you work.</p></div><span className="fc-labs-count">{completed} / {MISSIONS.length} COMPLETE</span></section>
    <div className="fc-mission-grid">{days.slice(0,4).flatMap((d)=>missionsForDay(d.id).map((m)=>{
      ...
      return <article key={m.id} className={complete?'complete':''} style={{'--day':d.color} as CSSProperties}>
        ...
        <div className="fc-mission-foot"><span><Flag size={11}/>{m.output}</span><button onClick={()=>launch(m.id,m.workspace,m.steps[0]?.module)}><MonitorPlay size={13}/>{complete?'Review':started?'Resume':'Start'}<ArrowRight size={12}/></button></div>
      </article>;
    }))}</div></div>;
}
...
function TrainerConsole({ dayIndex, setDayIndex, onBack, onPresent, onOpenWorkspace, onMaterial, allMaterials, onScoring, onAllMaterials }: { dayIndex:number; setDayIndex:(i:number)=>void; onBack:()=>void; onPresent:()=>void; onOpenWorkspace:(id:WorkspaceId,module?:string)=>void; onMaterial:(m:Material)=>void; allMaterials:Array<{material:Material;day:CourseDay}>; onScoring:()=>void; onAllMaterials:()=>void }) {
  const day = VOLVE_DAYS[dayIndex];
  const session = useSession();
