...
-- Lists ALL registered users (only the caller is excluded), paginated and
-- name-searchable. `rel` tells the UI whether you're already friends / pending.
create or replace function public.search_users(p_q text default '', p_limit int default 10, p_offset int default 0)
...
language sql stable security definer set search_path = public as $$
  ...
  order by p.display_name
  limit least(greatest(p_limit,1),30) offset greatest(p_offset,0);
$$;
