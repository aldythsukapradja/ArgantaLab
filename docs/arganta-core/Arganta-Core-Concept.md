...
| B5 | Preview + publishing | 🟢 **IMPL SHIPPED — [[../adr/0006-public-artifact-runtime]]** (Opus contract → Sonnet impl). `migration_artifact_publications.sql` (additive `artifact_publication` table, immutable-once-assigned slugs — live-verified republish reuses the same slug — + `publication_by_slug` the one anon-granted RPC), `publish_artifact` executor (re-validates the exact HTML before publishing — a second, independent check), `workers/build-artifact-runtime/` (the actual Cloudflare Worker: `router.js` pure+tested incl. a drift guard tying its CSP categories to `validate.js`'s `APPROVED_HOSTS`, `index.js` re-runs `validateHtml` server-side before every serve), Publish/Unpublish UI in `ArtifactCard.tsx` sharing the same executor a chat-driven publish would use. **Live-verified against the real Supabase project**, including invoking the actual Worker `fetch()` handler directly (200+CSP for a live slug, 404 for unknown/kind-mismatch/unrelated paths) — not mocked. `wrangler` isn't authenticated in this environment, so **Worker deploy is a founder action** (`wrangler login` + confirm the `arganta.app` zone + `wrangler deploy` + `wrangler secret put SUPABASE_ANON_KEY`); **go-live still needs the final Opus security review** ADR-0005/0006 named — this shipped the implementation, not the sign-off | **Opus contract ✅** → **Sonnet impl ✅** → **Opus go-live (next)** | public internet surface = new attack surface |

Sequencing: C1 ✅ → C2 ✅ → C3 ✅ → B1 ✅ → B2 ✅ → B3 ✅ → C4b ✅ → B4 ✅ → B5 impl ✅
→ **B5 go-live review + founder Worker deploy** → C5 → C6 → C7. C8 after C3 and again
after B5/C7. C9 parallel anytime.
