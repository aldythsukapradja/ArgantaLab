...
| C6 | Delegation | 🟡 **PROTOCOL FROZEN — [[../adr/0007-office-delegation-protocol]]** (Opus). The reconciliation C3's `consult_office` deferred: the 6 offices (`delegation.js`) are the canonical lens, the 27-agent roster (`agents.ts`) is the depth under each, mapped via the existing `AGENT_OFFICE`. A delegation resolves office→chief and runs the **grounded Sense→Compute→Match→Generate pipeline** (synthesizes over real computed facts, never free recall), degrading honestly to the C3 persona where no grounded pipeline exists. **The governance call:** grounding in live economy/growth data is a `confidential` op (ADR-0003) → its Generate step **stays local (Tier 0)**; if Tier 0 is down it shows the deterministic Match signals + "synthesis unavailable", **never** leaks confidential facts to the gateway. No nested office loops / no office-convenes-offices in v1. Trail must show grounded-vs-degraded. Sonnet rewires `runConsultOffice` + delegation-trail UI; external `consult_office` contract unchanged | **Opus protocol ✅** → **Sonnet** (impl next) | protocol is judgment, impl is wiring |
...
founder choice) → C5 ✅ → C6 protocol ✅ (ADR-0007) → **C6 impl (next)** → C7. C8 after C3 and again after B5/C7. C9 parallel
anytime.
