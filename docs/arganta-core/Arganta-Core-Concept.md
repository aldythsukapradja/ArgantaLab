...
| B4 | Portable components | 🟡 **B4a DESIGN SHIPPED** — [[B4a-Block-Design]] (frozen `--brand-*` theming mirroring makeBrand, `.blk-<id>` assembly-scoping, 19-block spec table mapped to B1's frozen categories) + 3 exemplar blocks in `docs/arganta-core/blocks/` (metric-card, chart-donut, footer). Remaining 16 blocks + harness + registry: Sonnet via `.claude/commands/build-portable-blocks.md` | **Fable** (block design ✅) → **Sonnet** (completion + registry) | blocks are aesthetic judgment; registry is wiring |
...
Sequencing: C1 ✅ → C2 ✅ → C3 ✅ → B1 ✅ → B2 ✅ → B3 ✅ → C4b ✅ → **B4 (next)**
→ B5 → C5 → C6 → C7. C8 after C3 and again after B5/C7. C9 parallel anytime.
