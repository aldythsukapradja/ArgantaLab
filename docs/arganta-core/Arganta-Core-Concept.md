...
| C3 | Tool loop | ✅ **SHIPPED, live-verified** — `lib/core/{runtime,tools,thread,index}.ts` wires the C1 loop to real models + real tools (image/speech/website/deck/brand/analyze/search_vault/consult_office/quota/ledger). Found + fixed 3 real bugs live: llm-proxy's `needsTools` filter could be bypassed by an exact-model request (Claude would silently hallucinate fake tool calls instead of erroring); Core's chat defaulted to `dataClass:'internal'`, which governance correctly blocked from the free tier; **(2026-07-15)** `needsTools`'s filter only excluded `shape==='anthropic'`, not genuine per-provider tool reliability — with Gemini/Groq keys never configured server-side, every tool-requiring request (including plain conversational messages, since the agent loop always offers tools) silently fell back to Cloudflare's free Llama, which accepts a `tools` body without erroring but hallucinates fake tool names (`greeting_response`, `generate_text`) instead of answering in text, burning all `maxSteps` — fixed by adding an honest `toolsCapable` flag per `router.js` catalog entry (`true` only for gemini/groq, matching `@arganta/ai/registry.js`'s existing client-side `capabilities.tools`) and filtering on it; live-verified: plain greetings now correctly return the honest "no live model reachable" degrade instead of hallucinating | **Sonnet** (Opus reviews the loop-termination/budget logic) | mechanical once C1 fixes the contract |
...
| C5 | Memory/RAG | ✅ **SHIPPED, live-verified** — Vault→`memory_chunk` sync (`vault/embed.ts`, chunked+embedded, manual "Sync to Core memory" button in VaultSettings — deliberately not automatic, keeps embedding cost predictable) + **auto-recall**: every chat turn now embeds the founder's message and injects the top relevant Vault chunks into the system prompt *before* the model even has to ask, surfaced honestly as an `auto_recall` tool-trail block. Deliberately gates auto-recall at a **tighter** dataClass ceiling (`internal`) than the existing on-demand `search_vault` tool (`confidential`) — background/automatic access earns less trust than an explicit founder-initiated tool call, live-verified: a confidential-tagged chunk is excluded at the auto-recall ceiling but reachable via `search_vault`. Added `memory_chunk_delete_by_ref` RPC (memory_chunk_upsert was insert-only, so re-syncing an edited note would have piled up stale duplicate chunks forever) — live-verified idempotent (re-sync of the same notes: 21 chunks both times, not 42). Retrieval quality live-verified strong: a real query against the real synced Vault returned the exactly-right note at 0.82 similarity, ranked #1 | **Sonnet** | wiring + tests |
| C6 | Delegation | ✅ **SHIPPED, live-verified — [[../adr/0007-office-delegation-protocol]]**. `runConsultOffice` (`lib/core/tools.ts`) rewired per ADR-0007: `roster` answers from static org metadata; `bridge`/`technology`/`legal` stay the C3 persona (now actually `dataClass:'internal'`-governed via `selectModel`, closing a latent gap — the old code called `ai.chat` with zero dataClass awareness at all); `operations`/`treasury` run the real `agents.ts` Sense→Compute→Match→Generate pipeline over live Supabase data, gated at `dataClass:'confidential'` — `selectModel` can only return a Tier-0 candidate at that class (`governance.js`), so "confidential stays local" is enforced by construction, not asserted. Trust signal travels as a `_(tag)_` prefix in the delegation summary (the frozen block only carries `{office,summary}`) — `DelegationCard` (new, `Message.tsx`) parses it back out into its own chip. **Found + fixed a real bug during verification**: Tier-0/WebLLM can *hang* rather than fail cleanly (the standing "WebLLM brittle" gap) — added a 30s timeout so a stuck local-model load degrades honestly instead of hanging the whole chat turn forever. **Also found the `delegation` block kind was never rendered in the chat UI at all** (dropped silently since C3) — added `DelegationCard`. Live-verified all 6 offices + auto-routing + invalid-office fallback against the real Supabase project: grounded offices showed real live numbers via the honest deterministic fallback with confidential data provably never leaving the device; persona offices honestly reported no live model | **Opus protocol ✅** → **Sonnet impl ✅** | protocol is judgment, impl is wiring |
...
| B5 | Preview + publishing | 🟢 **DEPLOYED AND LIVE — [[../adr/0006-public-artifact-runtime]]** (Opus contract → Sonnet impl → founder deploy). `build.arganta.app` is a real, resolving, publicly-reachable domain, routed to the Worker, secret configured. Founder walked through `wrangler login`→`deploy`→`secret put` step by step; deploy required an explicit go-ahead (blocked twice by the permission system for legitimate reasons — once citing the ADR-0006 review, once because a founder question hadn't been answered yet). **Real gap found on first live test:** a Worker route doesn't create a DNS record — `build.arganta.app` didn't resolve until the founder added an `A build.arganta.app 192.0.2.1` record (proxied). Full loop then proven on the real internet: publish → `curl` the actual public URL → real 200 + exact CSP → unpublish → confirmed 404 within seconds. **Founder made an informed, explicit call to defer the formal Opus go-live review** until after seeing it work end to end — understood and accepted that the anon key is already public/narrowly-scoped, real secrets never touch the Worker, and published content is genuinely public. Review should still happen before anything the founder actually cares about goes up | **Opus contract ✅** → **Sonnet impl ✅** → **founder deploy ✅** → **Opus go-live review (deferred by founder choice)** | public internet surface = new attack surface |

Sequencing: C1 ✅ → C2 ✅ → C3 ✅ → B1 ✅ → B2 ✅ → B3 ✅ → C4b ✅ → B4 ✅ → B5 ✅
(deployed, live at build.arganta.app; formal go-live review deferred by
founder choice) → C5 ✅ → C6 ✅ (ADR-0007, protocol + impl) → **C7 (next, blocked on ADR-0004's
