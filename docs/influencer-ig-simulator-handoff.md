...
| ~~**P4** (Sonnet)~~ **DONE** `bd1df293` | Reels tab + post overlay polish + pinned handling | ✅ verified: double-click opens the single-post overlay (media/reel-play-icon/no-media text fallback), close works, PINNED header renders in grid only (never Reels) |
| ~~**P5** (Sonnet)~~ **DONE** `bd1df293` | Supabase persistence of the plan (migration `migration_ig_plan.sql`) + posted-status readback from `published_to` | ⚠️ **RUN THE MIGRATION FIRST** — `supabase/migration_ig_plan.sql` hasn't been applied to the live project yet. Code paths are written and typecheck clean but cloud read/write/reconcile are unverified live (same dev-has-no-project gap as P3). Verify once connected: plan an item, refresh, confirm it survives; send it, wait ~12s after Post Studio publishes, confirm it flips to `posted`. |

All five phases shipped 2026-07-17. **Founder follow-up required before P5 is real:** run `supabase/migration_ig_plan.sql` in the ArgantaLab project, then connect `apps/hq/.env.local` if not already, and do one live send+publish+reconcile cycle to close the verification gap flagged in P3 and P5.
